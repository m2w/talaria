...
var talaria = (function (P) {
     ...
     */
    // FIXME: this is largly duplicated, join and use conditionals where needed instead?
        ...
        return getJSON(CONFIG.ISSUE_MAPPINGS).
            then(function (issueMappings) {
                ...
                return maybeGetCachedVersion(mapping.obj.permalink).
                        ...
                        displayCommentsForIssue(mapping.linkobj, issue);
                        return issue;
                    }).catch(function (error) {
                        mapping.obj.comments = [];
                        showErrorForIssue(mapping.linkobj, error, mapping.obj);
                        return mapping.obj;
                    });
    ...
    var showErrorForIssue = function(permalinkElement, error, issue) {
        var issueUrl = CONFIG.ISSUE_URL_ROOT + issue.id + '#comments',
            wrapper = wrapperTemplate(issue.id,
                                      issueUrl,
        ...
        var elem = document.querySelector('#talaria-wrap-' + issue.id + ' div.talaria-load-error');
        elem.classList.remove('hide');
        document.querySelector('#talaria-wrap-' + issue.id + ' div.talaria-comment-count').classList.add('hide');
        ...
        case 403:
            ...
            break;
        case 404:
            elem.innerHTML = 'Unable to find a matching issue for this content.';
            break;
        default:
    ...
    var displayCommentsForIssue = function(permalinkElement, issue) {
        var issueUrl = CONFIG.ISSUE_URL_ROOT + issue.id + '#comments',
            wrapper = wrapperTemplate(issue.id,
                                      issueUrl,
                                      issue.comments.length,
                                      (location.pathname === '/' ||
                                       CONFIG.PAGINATION_SCHEME.test(location.pathname))),
            commentHtml = issue.comments.map(commentTemplate).join('');

        parentArticle(permalinkElement).appendChild(wrapper);
        document.getElementById('talaria-comment-list-' + issue.id).innerHTML = commentHtml;
        addClickHandlers(issue.id, issue.permalink, issue.comments.length > 0);
    };
