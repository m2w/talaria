/*global document,P,sessionStorage,XMLHttpRequest,location*/
var talaria = (function (P) {
    ...
    var setPermalinkRegex = function () {
        ...
        case 'none':
            if (CONFIG.BACKEND === BACKENDS.COMMITS) {
    ...
    /*
     * Github API interaction - shared functions
     */
    var getRelevantMappings = function (allMappings) {
        var mappings = [],
            obj;

        for (var entry in allMappings) {
            if (allMappings.hasOwnProperty(entry)) {
                obj = allMappings[entry];
                var permalink = document.querySelector(CONFIG.PERMALINK_IDENTIFIER +
                                                       '[href="' + obj.permalink + '"]');
                if (permalink !== null) {
                    mappings.push({'obj': allMappings[entry],
                                   'linkobj': permalink});
                }
            }
        }
    ...
    var showMappingsError = function () {
    ...
    var showError = function (permalinkElement, error, sourceObj) {
        var errorMsg, url, wrapper;
        if (CONFIG.BACKEND === BACKENDS.GISTS) {
            url = CONFIG.GIST_URL_ROOT + sourceObj.id + '#comments';
        } else {
            url = CONFIG.ISSUE_URL_ROOT + sourceObj.id + '#comments';
        }
        wrapper = wrapperTemplate(sourceObj.id,
                                  url,
                                  sourceObj.comments.length,
                                  (location.pathname === '/' ||
                                   CONFIG.PAGINATION_SCHEME.test(location.pathname)));

        parentArticle(permalinkElement).appendChild(wrapper);

        var elem = document.querySelector('#talaria-wrap-' + sourceObj.id + ' div.talaria-load-error');
        elem.classList.remove('hide');
        document.querySelector('#talaria-wrap-' + sourceObj.id + ' div.talaria-comment-count').classList.add('hide');
        ...
        case 404:
            if (CONFIG.BACKEND === BACKENDS.GISTS) {
                errorMsg = 'Unable to find a matching gist.';
            } else {
                errorMsg = 'Unable to find a matching issue for this content.';
            }
    ...
    var displayComments = function (permalinkElement, sourceObj) {
        var url, wrapper, commentHtml;

        if (CONFIG.BACKEND === BACKENDS.GISTS) {
            url = CONFIG.GIST_URL_ROOT + sourceObj.id + '#comments';
        } else if (CONFIG.BACKEND === BACKENDS.ISSUES) {
            url = CONFIG.ISSUE_URL_ROOT + sourceObj.id + '#comments';
        } else {
            var lastCommit = latest(sourceObj.commits);
            url = CONFIG.COMMIT_URL_ROOT +
                lastCommit.sha + '#all_commit_comments';
            sourceObj.id = lastCommit.sha;
        }

        wrapper = wrapperTemplate(sourceObj.id, url, sourceObj.comments.length,
                                  (location.pathname === '/' ||
                                   CONFIG.PAGINATION_SCHEME.test(location.pathname)));
        commentHtml = sourceObj.comments.map(commentTemplate).join('');

        parentArticle(permalinkElement).appendChild(wrapper);
        document.getElementById('talaria-comment-list-' + sourceObj.id).innerHTML = commentHtml;
        addClickHandlers(sourceObj.id, sourceObj.permalink, sourceObj.comments.length > 0);
    };
    ...
    var retrieveCommitBasedComments = function () {
        return grabPermalinks().map(function (permalink) {
            return maybeGetCachedVersion(permalink.getAttribute('href')).then(function (cacheData) {
                displayComments(permalink, cacheData);
    ...
    /*
     * Github API interaction - Issue-based
     */
    var retrieveIssueBasedComments = function () {
        return getJSON(CONFIG.MAPPINGS).
            then(function (issueMappings) {
                return getRelevantMappings(issueMappings);
            }).map(function (mapping) {
                return maybeGetCachedVersion(mapping.obj.permalink).
                    catch(function () {
                        return getIssueComments(mapping.obj);
                    }).then(function (issue) {
                        displayComments(mapping.linkobj, issue);
                        return issue;
                    }).catch(function (error) {
                        mapping.obj.comments = [];
                        showError(mapping.linkobj, error, mapping.obj);
                        return mapping.obj;
                    });
    ...
    var getIssueComments = function (issue) {
        return getJSON(CONFIG.ISSUE_API_ENDPOINT + issue.id + '/comments').
            then(function (comments) {
                issue.comments = comments;
                cacheCommentData(issue.permalink, issue);
                return issue;
            });
    };
        ...
        return getJSON(CONFIG.MAPPINGS).
            then(function (gistMappings) {
                ...
                return maybeGetCachedVersion(gist.permalink).
                        ...
                        displayComments(mapping.linkobj, gist);
                        return gist;
                    }).catch(function (error) {
                        // 404 or 403
                        gist.comments = [];
                        showError(mapping.linkobj, error, gist);
                        return gist;
                    });
            }, {concurrency: 2}).catch(function () {
                showMappingsError();
    ...
    var updateConfig = function (config) {
        ...
        CONFIG.COMMIT_URL_ROOT = 'https://github.com/' +
        ...
        CONFIG.ISSUE_API_ENDPOINT = 'https://api.github.com/repos/' + CONFIG.GITHUB_USERNAME +
            '/' + CONFIG.REPOSITORY_NAME + '/issues/';
        CONFIG.ISSUE_URL_ROOT = 'https://github.com/' +
            CONFIG.GITHUB_USERNAME + '/' + CONFIG.REPOSITORY_NAME + '/issues/';
    ...
    var initialize = function (config) {
        ...
        case (BACKENDS.ISSUES):
            return retrieveIssueBasedComments();
        default:
