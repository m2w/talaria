/*global document,$,P,sessionStorage,location,console*/
var talaria = (function ($, P) {
    ...
    var grabCommitsForFile = function(permalink) {
        ...
        return P.resolve($.getJSON(CONFIG.COMMIT_API_ENDPOINT, {
            ...
            return P.reject('x-ratelimit');
        });
    };

    var grabCommentsForCommit = function(commit) {
        // TODO: should this quietly handle errors or escalate 403s?
    ...
    var retrieveGistBasedComments = function() {
        return P.resolve($.getJSON(CONFIG.GIST_MAPPINGS)).
            then(function (gistMappings) {
                ...
                return maybeGetCachedVersion(mapping.gist.permalink).
                    catch(function (reason) {
                        // cache miss
                        // TODO: refactor into its own method
                        return P.resolve($.getJSON('https://api.github.com/gists/' + mapping.gist.id + '/comments')).
                            then(function (comments) {
                                gist.comments = comments;
                                cacheCommentData(mapping.gist.permalink, mapping.gist);
                                return gist;
                            });
                    }).then(function (gist) {
